...
go env -w GOTOOLCHAIN=go1.26.8+auto # https://github.com/golang/go/issues/75031
...
"${REPO_ROOT}/bin/gotestsum" --junitfile="${REPO_ROOT}/bin/junit.xml" -- \
    -coverprofile="${REPO_ROOT}/bin/unit.cov" -covermode count -coverpkg sigs.k8s.io/kubetest2/... ./...
...
if [[ -n "${ARTIFACTS:-}" ]]; then
  cp "${REPO_ROOT}/bin/junit.xml" "${REPO_ROOT}/bin/filtered.cov" "${REPO_ROOT}/bin/filtered.html" "${ARTIFACTS:?}/"
fi
