...
```
kubetest2 <deployer> [Flags] [DeployerFlags] --test=<tester> -- [TesterArgs]
```
...
**Example**: run multiple testers in sequence

```
kubetest2 kind \
  --up \
  --down \
  --test=exec -- ./my-test-script.sh \
  --test=ginkgo \
  -- \
  --focus-regex='\[Conformance\]'
```

Each `--test=<tester>` is followed by `--` and its arguments. Multiple testers run in sequence.
