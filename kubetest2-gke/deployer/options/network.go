...
type NetworkOptions struct {
	...
	ExtraNetwork                 []string `flag:"~extra-network" desc:"create an extra network. repeat the flag for another network. options as key=value&key=value..."`
	SSHProxyInstanceName         string   `flag:"~ssh-proxy-instance-name" desc:"If set, will result in proxing the ssh connections via the provided instance name while running tests."`
